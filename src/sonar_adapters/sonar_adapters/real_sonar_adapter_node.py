...
class RealSonarAdapterNode(Node):
    def __init__(self) -> None:
        super().__init__("real_sonar_adapter")

        self.declare_parameter("input_topic", "/real_sonar/image_raw")
        self.declare_parameter("output_topic", "/sonar/std/frame")
        self.declare_parameter("source_id", "real_sonar")
        self.declare_parameter("projection", "polar")
        self.declare_parameter("range_min_m", 0.2)
        self.declare_parameter("range_max_m", 80.0)
        self.declare_parameter("fov_rad", float(math.pi))
